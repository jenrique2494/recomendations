...
def buscador_principal(producto_buscar,lista_productos):
    ...
    rows["features"]=rows["combined_features"]
    ...
    similar_products =  pd.DataFrame(list(enumerate(cosine_sim[product_index])),columns=['index', 'calificacion'])

    productos_similares= similar_products.drop(similar_products[similar_products['calificacion']<=0].index)
    
    #mostrar los productos similares a partir del mas parecido
    sorted_similar_products = productos_similares.sort_values('calificacion',ascending=False)

    #muestra de los productos mas parecidos
    i=0
    recomendaciones = []
    for index, row in sorted_similar_products.iterrows():
        if i>0:
            dfnew ={'bank_id': int(get_id_from_index(index)), 'name': get_title_from_index(index), 'description': get_home(index), 'calificacion': row['calificacion']}
